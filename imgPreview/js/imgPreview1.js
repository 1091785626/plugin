	...
	$.fn.imgPreview = function( options ){
		...
		var _private = {
			init:function(obj){//初始化
				this.overlay 	= $('<div class="galleryOverlay" id='+'gallery_'+settings.id+'></div>');
				this.slider 	= $('<div class="gallerySlider"></div>');
				this.prevArrow	= $('<a class="prevArrow"></a>');
				this.nextArrow 	= $('<a class="nextArrow"></a>');
				this.pageSpan 	= $('<span class="pagelimit"></span>');
				this.close 		= $('<span class="galleryClose">&#10005;</span>');
			...
			bindEvent:function(){
				var _this = this;
				this.slider.append(this.placeholders).on('click',function(e){
					_this.hideOverlay();			
				});
				...
				this.items.on('click', function(e){
					...
					_this.index = _this.items.index(this);//?
					_this.showOverlay(_this.index);
					_this.showImage(_this.index);
					
					_this.calcPages(_this.items,_this.index);
					// 预加载下一个图像
			...
			hideOverlay:function(){
				...
				this.overlayVisible = false;
				$(document).data("overlayVisible",this.overlayVisible);
